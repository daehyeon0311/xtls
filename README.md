...
GUI를 쓰시려면 `pip install -e ".[gui]"` — `streamlit`이 추가됩니다.

## 실행 방법 1 — GUI

파라미터를 슬라이더로 조정하며 스펙트럼이 어떻게 움직이는지 바로 보는 방식입니다.
...
- **실시간 조정** — Δ, U_dd, U_dc, 10Dq, 혼성화, Slater 축소 인자, 브로드닝
- **실험 데이터 겹치기** — 측정 스펙트럼을 올리면 RMS 편차가 함께 표시됩니다
- **곡선 고정** — 파라미터를 바꿔가며 이전 곡선 위에 겹쳐 감도를 봅니다
- **XAS·XPS 동시 표시** — 한 파라미터 세트가 둘 다 만족하는지 확인
- **입력 파일로 내보내기** — 맞춘 값을 `inputs/*.py`로 저장해 `run.py`로 재현

계산 품질은 세 단계입니다. 탐색(h=1, 거친 격자)은 슬라이더를 끌 만한 속도이고,
확정 후 최종 품질로 한 번 돌리는 흐름을 의도했습니다.
...
```
app.py               Streamlit GUI — 슬라이더 피팅, 실험 데이터 비교
run.py               명령줄 진입점 — 입력 파일과 분광법 선택
